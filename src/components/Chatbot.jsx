...
//       if (user) {
//         setUserId(user.uid);   // 🔥 each student has their own chat
//       } else {
//         setUserId(null);
//       }
//     });
...
export default function Chatbot() {   // ✅ removed { docId } prop (🔥 changed)
  const [messages, setMessages] = useState([]);
  const [input, setInput] = useState("");
  const [loading, setLoading] = useState(false);
  const [userId, setUserId] = useState(null);   // 🔥 track logged-in student
  const listRef = useRef(null);
